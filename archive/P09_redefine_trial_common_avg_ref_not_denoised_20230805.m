...
ARTIFACT_CRIT = 'E'; % 70-150hz high gamma; identifier for the criteria implemented in this script
...
% use this trial duration if one is not specified in the trials annot table
...
default_trialdur_max_if_empty = 15;  % sec
default_iti_if_empty = 0.5; % sec
...
for itrial = 1:height(epoch)
    if isnan(epoch.duration(itrial))
        if ~[itrial == height(epoch)]
            epoch.ends(itrial)= min([epoch.starts(itrial) + default_trialdur_max_if_empty, epoch.ends(itrial+1) - default_iti_if_empty]);
        elseif itrial == height(epoch) % last trial
            epoch.ends(itrial)= epoch.starts(itrial) + default_trialdur_max_if_empty;
        end
...
save([PATH_FIELDTRIP filesep 'sub-' SUBJECT '_ses-' SESSION '_task-' TASK '_ft-raw-filt-trial-ar-ref-', ARTIFACT_CRIT, '.mat'],'D_trial_ref','-v7.3');
