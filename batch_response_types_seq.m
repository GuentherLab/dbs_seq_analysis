...
compiled_responses_filepath = [PATH_RESULTS, filesep, 'resp_all_subjects_', resp_signal]; 
...
for isub = 1:nsubs
    clearvars -except subs compiled_responses_filepath nsubs isub resp_signal ARTIFACT_CRIT
    SUBJECT = subs.subject{isub}
    response_types_seq()
    savefile = [PATH_RESULTS, filesep, SUBJECT '_responses_' resp_signal];
...
for isub = 1:nsubs
    SUBJECT = subs.subject{isub};
    load([PATH_RESULTS, filesep, SUBJECT, '_responses_', resp_signal],'resp','trials')
