...
if ~raw_has_nan && ismember(method,{'CAR','LAR','VAR'})
 ...
 %% Common Median Reference 
  elseif ismember(method,{'CMR'})
    ...
    %% Common Median Rereference with SVD subtraction
    elseif ismember(method,{'CMR_SVD'})
    ref = raw;

    % concatonate trials
    temp = [];
    for t=1:numel(raw.trial)
        temp = [temp, raw.trial{t}];
    end

    % find mean
    commmean = trimmean(temp,percent,2);

    for t=1:numel(raw.trial)
      % for g=1:numel(ug)
        %calculating groups common median 
        % nanmask
        % mask = isnan(raw.trial{t}(group==ug(g),:));
        mask = isnan(raw.trial{t});
        % replace with 0
        % y = raw.trial{t}(group==ug(g),:); % 16 x n_trial
        y = raw.trial{t}; 
        y(mask) = 0;
        
        % find mean and subtract
        % commmean = trimmean(y,percent,2);
        y = y - commmean;    

        % SVD
        [u, s, v] = svd(y',0);
        % disp(v(:,1)) % first col of v matrix should be constant if all elecs in
        % group are similar
        commmed = v(:,1) * s(1) * u(:,1)';
        % commmed = nanmedian(raw.trial{t}(group==ug(g),:),1);
        % ref.trial{t}(group==ug(g),:) = raw.trial{t}(group==ug(g),:) - commmed;
        ref.trial{t} = raw.trial{t} - commmed;
      % end
    end 

    %% Common Median Rereference with SVD subtraction
    elseif ismember(method,{'CMR_SVD_tr'})
    ref = raw;
    ...
    for t=1:numel(raw.trial)
      % for g=1:numel(ug)
        ...
        % commmed = nanmedian(raw.trial{t}(group==ug(g),:),1);
        % ref.trial{t}(group==ug(g),:) = raw.trial{t}(group==ug(g),:) - commmed;
        ref.trial{t} = raw.trial{t} - commmean - commmed;
      % end
    end 

    %%
    elseif ismember(method,{'CMR_SVD_c2'})
    ref = raw;
    ...
    for t=1:numel(raw.trial)
        temp = raw.trial{t};
        tr_lengths = [tr_lengths, size(temp,2)];

        all_tr = [all_tr, temp];        
    end

    % find mean and subtract
    commmean = trimmean(all_tr,percent,2);
    all_tr = all_tr-commmean;
    ...
    [u, s, v] = svd(all_tr',0);
    commmed = v(:,1) * s(1) * u(:,1)';
    % commmed = v(:,1:2) * s(1:2,1:2) * u(:,1:2)';

    % subtract svd from raw trial
    for t=1:numel(raw.trial)     
        start_index = 1 + sum(tr_lengths(1:t-1));
        end_index = sum(tr_lengths(1:t));

        % ref.trial{t} = raw.trial{t} - commmed(:,start_index:end_index);  
        ref.trial{t} = raw.trial{t} - commmean - commmed(:,start_index:end_index);
    end 

    %% Common Median Rereference Subtracted Signal (created by rohan
    elseif ismember(method,{'CMR_sub'})
    ref = raw;
    for t=1:numel(raw.trial)
      for g=1:numel(ug)
        %calculating groups common median 
        commmed = nanmedian(raw.trial{t}(group==ug(g),:),1);
        % ref.trial{t}(group==ug(g),:) = raw.trial{t}(group==ug(g),:) - commmed; % edited by rohan
        commmed = repmat(commmed, size(ref.trial{t}(group==ug(g),:),1), 1);
        ref.trial{t}(group==ug(g),:) = commmed; % edited by rohan
      end
    end 
    
  elseif ismember(method, {'CMR_ref'})
      ref=raw;
        
      for t=1:numel(raw.trial)
          for g=1:numel(ug)
              commmed = nanmedian(raw.trial{t}(group==ug(g),:),1); % reference signal
              e = [ones(size(commmed)); commmed];  % design matrix for regression (2,n)
              
              for n=find(group==ug(g),1,'first'):find(group==ug(g),1,'last')
                valid = ~isnan(raw.trial{t}(n,:)); % identify non-nan indices in elec
                ref.trial{t}(n,:) = ref.trial{t}(n,:) - ref.trial{t}(n,valid)/e(:,valid)*e;  % regress out the reference signal from each electrode in group
                % ref.trial{t}(group==ug(g),:) = ref.trial{t}(group==ug(g),:) - commmed(:,valid)/e(:,valid)*e;  % regress out the reference signal from each electrode
              end
          end
      end
